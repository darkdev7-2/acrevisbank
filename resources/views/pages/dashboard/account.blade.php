    ...
    <div class="p-4 sm:p-6 lg:p-8">
        ...
        <div class="bg-white rounded-lg border border-slate-200 shadow-sm p-6 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    <p class="text-xs font-medium text-slate-500 mb-1">{{ $t['account_number'] }}</p>
                    <p class="font-mono font-semibold text-slate-900">{{ $account->account_number }}</p>
                </div>
                <div>
                    <p class="text-xs font-medium text-slate-500 mb-1">{{ $t['iban'] }}</p>
                    <p class="font-mono text-sm font-semibold text-slate-900">{{ trim($account->formatted_iban) }}</p>
                </div>
                <div>
                    <p class="text-xs font-medium text-slate-500 mb-1">{{ $t['available'] }}</p>
                    <p class="font-semibold text-emerald-600">{{ number_format($account->available_balance, 2, '.', "'") }} {{ $account->currency }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg border border-slate-200 shadow-sm p-5 mb-6">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-base font-semibold text-slate-900">{{ $t['export'] }} & {{ $t['filters'] }}</h3>
                <button onclick="toggleFilters()" type="button" class="text-sm text-blue-600 hover:text-blue-700 font-medium flex items-center">
                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div id="filterForm" class="hidden mb-5">
                <form method="GET" action="{{ route('dashboard.account', ['locale' => $currentLocale, 'id' => $account->id]) }}" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <!-- Date From -->
                        <div>
                            <label for="date_from" class="block text-xs font-medium text-slate-700 mb-1.5">{{ $t['date_from'] }}</label>
                            <input type="date"
                                   name="date_from"
                                   id="date_from"
                        ...
                        <!-- Date To -->
                        <div>
                            <label for="date_to" class="block text-xs font-medium text-slate-700 mb-1.5">{{ $t['date_to'] }}</label>
                            <input type="date"
                                   name="date_to"
                                   id="date_to"
                        ...
                        <!-- Type -->
                        <div>
                            <label for="type" class="block text-xs font-medium text-slate-700 mb-1.5">{{ $t['type'] }}</label>
                            <select name="type"
                                    id="type"
                                    class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        <!-- Min Amount -->
                        <div>
                            <label for="min_amount" class="block text-xs font-medium text-slate-700 mb-1.5">{{ $t['min_amount'] }}</label>
                            <input type="number"
                                   name="min_amount"
                                   id="min_amount"
                                   step="0.01"
                        ...
                        <!-- Max Amount -->
                        <div>
                            <label for="max_amount" class="block text-xs font-medium text-slate-700 mb-1.5">{{ $t['max_amount'] }}</label>
                            <input type="number"
                                   name="max_amount"
                                   id="max_amount"
                                   step="0.01"
                        ...
                        <!-- Search -->
                        <div>
                            <label for="search" class="block text-xs font-medium text-slate-700 mb-1.5">{{ $t['search'] }}</label>
                            <input type="text"
                                   name="search"
                                   id="search"
                    ...
                    <div class="flex gap-3 pt-2">
                        ...
                        <a href="{{ route('dashboard.account', ['locale' => $currentLocale, 'id' => $account->id]) }}"
                           class="inline-flex items-center px-4 py-2 bg-slate-100 text-slate-700 text-sm rounded-lg hover:bg-slate-200 transition-colors font-medium">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('dashboard.transactions.export.pdf', ['locale' => $currentLocale, 'accountId' => $account->id]) }}?{{ http_build_query(request()->except('page')) }}"
                   target="_blank"
                   class="inline-flex items-center px-3 py-2 bg-slate-100 text-slate-700 text-sm rounded-lg hover:bg-slate-200 transition-colors font-medium border border-slate-200">
                ...
                <a href="{{ route('dashboard.transfer', ['locale' => $currentLocale]) }}"
                   class="inline-flex items-center px-3 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 transition-colors font-medium ml-auto">
                    <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if($hasFilters)
                <div class="mt-4 pt-4 border-t border-slate-200">
                    <div class="flex flex-wrap gap-2 items-center">
                        <span class="text-xs font-medium text-slate-700">{{ $t['active_filters'] }}</span>
        ...
        <div class="flex items-center justify-between mb-5">
            <h2 class="text-xl font-bold text-slate-900">{{ $t['transactions'] }}</h2>
        </div>
        ...
        <div class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
                            ...
                            @foreach($transactions as $transaction)
                                <tr class="hover:bg-slate-50 transition-colors">
                                    <td class="px-5 py-4 whitespace-nowrap text-sm text-slate-900">
                                        ...
                                        <span class="block text-xs text-slate-500 mt-0.5">{{ $transaction->transaction_date->format('H:i') }}</span>
                                    </td>
                                    <td class="px-5 py-4 text-sm">
                                        ...
                                        @if($transaction->recipient_iban)
                                            <div class="text-xs text-slate-400 font-mono mt-0.5">{{ $transaction->recipient_iban }}</div>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-5 py-4 whitespace-nowrap text-sm text-right font-semibold">
                                        <span class="{{ $transaction->type === 'debit' ? 'text-red-600' : 'text-emerald-600' }}">
                                            {{ $transaction->formatted_amount }}
                                        </span>
                                    </td>
                                    <td class="px-5 py-4 whitespace-nowrap text-sm text-right font-medium text-slate-900">
            ...
            @else
                <div class="px-5 py-16 text-center">
                    ...
                    <h3 class="mt-3 text-base font-semibold text-slate-900">{{ $t['no_transactions'] }}</h3>
                    <p class="mt-1 text-sm text-slate-500">{{ $t['no_transactions_text'] }}</p>
                </div>
            @endif
        </div>
    </div>
